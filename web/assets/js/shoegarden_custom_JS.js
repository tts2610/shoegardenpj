$(document).ready(function () {
    ...
    var minPrice = $("#fs-shop-content , #fs-shop-content-sub-category, #fs-shop-discount").find("#fs-price-from-text").text();
    var maxPrice = $("#fs-shop-content , #fs-shop-content-sub-category, #fs-shop-discount").find("#fs-price-to-text").text();


    $("#fs-shop-content #slider , #fs-shop-content-sub-category #slider, #fs-shop-discount #slider ").rangeSlider({
        bounds: {min: Number(minPrice), max: Number(maxPrice)},
        formatter: function (val) {
            var value = Math.round(val * 100) / 100,
                    decimal = value - Math.round(val);
            return decimal == 0 ? value.toString() + ".0" : value.toString();
        },
        defaultValues: {min: Number(minPrice), max: Number(maxPrice)}
    });

    var fromPrice;
    var toPrice;

    $("#fs-shop-content #slider , #fs-shop-content-sub-category #slider, #fs-shop-discount #slider").bind("valuesChanging", function (e, data) {
        fromPrice = data.values.min;
        toPrice = data.values.max;
        $(this).find("#fs-price-from").val(fromPrice)
        $(this).find("#fs-price-to").val(toPrice);
    });

    //REFRESH BUTTON
    $("#fs-shop-content .refresh-btn").click(function () {

        $("#fs-shop-content #slider").rangeSlider("values", minPrice, maxPrice);
        $(this).attr("fs-filter", "yes");
        var page = 1;
        var fromPrice = minPrice
        var toPrice = maxPrice
        var cateID = $("#fs-shop-content").find("#slider #fs-price-from").attr("fs-category");
        ...
        if (fromPrice != "") {
            $("#fs-price-from-text").text(Math.round(fromPrice * 100) / 100);
        }

        if (toPrice != "") {
            $("#fs-price-to-text").text(Math.round(toPrice * 100) / 100);
        }
    ...
    $("#fs-shop-content-sub-category .refresh-btn").click(function () {
        $("#fs-shop-content-sub-category #slider").rangeSlider("values", minPrice, maxPrice);
        $(this).attr("fs-filter", "yes");
        var page = 1;
        var fromPrice = minPrice
        var toPrice = maxPrice
    ...
    $("#fs-shop-discount .refresh-btn").click(function () {

        $("#fs-shop-discount #slider").rangeSlider("values", minPrice, maxPrice);
        $(this).attr("fs-filter", "yes");
        var page = 1;
        var fromPrice = minPrice
        var toPrice = maxPrice
        ...
        $("#fs-filter-price-error").text("");
        $.ajax({
            url: "ajax/getNumberOfProductsByFilter_OfDiscount.html",
            method: "POST",
            data: {
                fromPrice: fromPrice,
                toPrice: toPrice,
            ...
            success: function (numberOfProducts) {
                ...
                $.ajax({
                    url: "ajax/productPaginationForDiscount.html",
                    method: "POST",
                    data: {
                        page: page,
                        itemPerPage: itemPerPage,
                    ...
                    success: function (response) {
                        setTimeout(function () {
                            ...
                            if (response.length == 0) {
                                ...
                                //Change pagination
                                var pagination = "<li><span class=\"fs-page-number fs-page-number-active\" fs-page-number=\"1\" fs-category=\"" + 0 + "\">1</span></li>";
                                if (numberOfPages > 1) {
                                    for (var i = 2; i <= numberOfPages; i++) {
                                        pagination += "<li><span class=\"fs-page-number\" fs-page-number=\"" + i + "\" fs-category=\"" + 0 + "\">" + i + "</span></li>";
                                    }
                                }
    ...
    $(".fs-wl-add-detail").click(function () {
        ...
        if (input.val() != "") {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response == "1") {
                        $('#error-cart-product-modal').html(errorHeadSuccess + "ADD PRODUCT TO WISHLIST SUCCESSFULLY!" + errorFoot);
                    } else if (response == "2") {
                        $('#error-cart-product-modal').html(errorHead + "THIS PRODUCT IS AVAILABLE IN YOUR WISH LIST" + errorFoot);
                    }
                }
            });
...
//                    window.location = window.location.href;
    ...
    $("#fs-product-detail-page").on("click", "#fs-product-detail-wl", function () {
        var userID = $(this).attr("fs-userID");
        var input = $("input[name='emailUser']");
        var proID = $(this).attr("fs-productID");
        if (input.val() != "") {
            //Có session
                ...
                success: function (response) {
                    if (response == "1") {
                        ...
                        $("#error-product-detail").html("<div class=\"alert alert-success\">\n" +
                                "<strong>ADD PRODUCT TO WISH LIST SUCCESSFULLY</strong>\n" +
                                "</div>");
                    } else if (response == "2") {
                        $("#error-product-detail").html("<div class=\"alert alert-danger\">\n" +
                                "<strong>THIS PRODUCT IS AVAILABLE IN YOUR WISH LIST</strong>\n" +
                                "</div>");
                    }
                }
            });
    ...
    $(".fs-compare-detail").click(function () {
        var productID = proID;
        var errorHead = "<div class=\"alert alert-danger\"><strong>";
        var errorHeadSuccess = "<div class=\"alert alert-success\"><strong>";
        var errorFoot = "</strong></div>";
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response == "1") {
                    $('#error-cart-product-modal').html(errorHeadSuccess + "ADD PRODUCT TO COMPARELIST SUCCESSFULLY!" + errorFoot);
                    $.ajax({
                    ...
                    $('#error-cart-product-modal').html(errorHead + "THIS PRODUCT IS AVAILABLE IN YOUR COMPARELIST" + errorFoot);
                }
            }
        });
    });
...
//            $('html,body').scrollTop(0);          
            ...
            if (ratingVal == "") {
                ...
                swal({
                    title: "Are you sure?",
                    text: "Continue without rating stars?",
                    type: "warning",
...
//                                                });
    ...
    $("#fs-shop-content").on("click", "#fs-btn-filter-price", function () {
        $(this).attr("fs-filter", "yes");
        var page = 1;
        var fromPrice = $("#fs-shop-content").find("#slider #fs-price-from").val();
        var toPrice = $("#fs-shop-content").find("#slider #fs-price-to").val()
        var cateID = $("#fs-shop-content").find("#slider #fs-price-from").attr("fs-category");
        var sortBy = $("#fs-sort-product-by").val(); //1: Newest; 2: Low to High Price; 3: High to Low Price
        var itemPerPage = $("#fs-number-of-item-on-page").val();
    ...
    $("#fs-number-of-item-on-page").selectBoxIt();
    /* AJAX ON CLICK PAGE */
    $("#fs-shop-discount").on("click", ".fs-page-number", function () {
        if (!$(this).hasClass("fs-page-number-active")) {
            $(".fs-page-number").removeClass("fs-page-number-active");
            $(this).addClass("fs-page-number-active");
            var page = $(this).attr("fs-page-number"); //Số trang hiện tại
    ...
    /* AJAX ON CHANGE SORT PRODUCT BY  */
    $("#fs-shop-discount").on("change", "#fs-sort-product-by", function () {
        var sortBy = $(this).val(); //1: Newest; 2: Low to High Price; 3: High to Low Price
    ...
    /* AJAX ON CHANGE NUMBER OF PRODUCT PER PAGE */
    $("#fs-shop-discount").on("change", "#fs-number-of-item-on-page", function () {
        var sortBy = $("#fs-sort-product-by").val(); //1: Newest; 2: Low to High Price; 3: High to Low Price
        var itemPerPage = $("#fs-number-of-item-on-page").val();
        var page = 1;
        var numberOfProducts = parseInt($("#fs-number-of-products").text());
    ...
    /* FILTER PRODUCT BY PRICE */
    $("#fs-shop-discount").on("click", "#fs-btn-filter-price", function () {
        $(this).attr("fs-filter", "yes");
        var page = 1;
        var fromPrice = $("#fs-shop-discount").find("#slider #fs-price-from").val();
        var toPrice = $("#fs-shop-discount").find("#slider #fs-price-to").val()
    ...
    /* FILTER PRODUCT BY COLOR */
    $("#fs-shop-discount").on("click", "#fs-color-checkbox", function () {
        if (!$(this).is('.checked')) {
            $(this).addClass('checked');
            colorFilterArr.push($(this).attr("name"));
        } else {
    ...
    /* FILTER PRODUCT BY SIZE */
    $("#fs-shop-discount").on("change", ".fs-size-checkbox", function () {
        if (this.checked) { //Check
            sizeFilterArr.push($(this).val());
        } else { //Bỏ Check
            var index = sizeFilterArr.indexOf($(this).val());
...
//    });
    ...
    $("#fs-shop-content-sub-category").on("click", "#fs-btn-filter-price", function () {
        ...
        $(this).attr("fs-filter", "yes");
        var page = 1;
        var fromPrice = $("#fs-shop-content-sub-category").find("#slider #fs-price-from").val();
        var toPrice = $("#fs-shop-content-sub-category").find("#slider #fs-price-to").val()
        var subCateID = $("#fs-shop-content-sub-category").find("#slider #fs-price-from").attr("fs-subCategory");
        var sortBy = $("#fs-sort-product-by").val(); //1: Newest; 2: Low to High Price; 3: High to Low Price
        var itemPerPage = $("#fs-number-of-item-on-page").val();
...
//    });
    ...
    function PassWrong(password) {
        ...
        if ($("#loginModal").modal('show')) {
            $("#fs-error-show").text("Invalid Password!");
            $("#fs-pass-login-user").focus();
            var div = $("#fs-pass-login-user").closest("div.fs-pass-user");
            div.removeClass("has-success");
            $("#glypcn-fs-login-user").remove();
    ...
    $("#fs-shop-discount").on('click', '.fs-wl-add-bra', function () {
        var userID = $(this).attr("fs-userID");
        var productID = $(this).attr("fs-productID");
        var input = $("input[name='emailUser']");
        if (input.val() != "") {
    ...
    $("#fs-product-detail-page").on('click', '.fs-wl-add-pddt', function () {
        var userID = $(this).attr("fs-userID");
        var productID = $(this).attr("fs-productID");
        var input = $("input[name='emailUser']");
        if (input.val() != "") {
...
//    });
    ...
    $("#fs-shop-discount").on('click','.fs-wl-add-sub',function () {
        var userID = $(this).attr("fs-userID");
        var productID = $(this).attr("fs-productID");
        var input = $("input[name='emailUser']");
        if (input.val() != "") {
    ...
    //compare latest product
    $(".fs-cp-lsp").click(function () {
    ...
    //compare featured product
    $(".fs-cp-fp").click(function () {
    ...
    $("#fs-shop-discount").on('click', '.fs-cp-bra', function () {
    ...
    //compare product(brand list)
    $("#fs-product-detail-page").on('click', '.fs-cp-pddt', function () {
    ...
    $("#fs-shop-discount").on('click','.fs-cp-cat',function () {
    ...
    //delete single compare table

    $(".fs-compare-del").click(function () {

        var productID = $(this).attr("fs-compare-del-id");

        $.ajax({
            url: "ajax/comparedelete.html",
            method: "POST",
            data: {
    ...
    //deleteall compare table

    $("#comparedeleteall").click(function () {
