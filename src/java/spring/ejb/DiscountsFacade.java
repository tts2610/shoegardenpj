...
public class DiscountsFacade extends AbstractFacade<Discounts> implements DiscountsFacadeLocal {
    ...
    public List<Discounts> selectTop3Discount(){
       ...
       // System.err.println(date);
        Query q = getEntityManager().createNativeQuery("SELECT TOP 3 * FROM discounts d where d.dateBegin<= '"+date+"' AND d.dateEnd>='"+date+"' AND d.discount <= (SELECT MAX(discount) from discounts) order by d.discount desc",Discounts.class);
